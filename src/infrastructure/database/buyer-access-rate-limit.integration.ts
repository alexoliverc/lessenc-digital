...
const windowStart = new Date("2026-09-13T16:00:00.000Z");
...
function input(
  ...
  return {
    scope: overrides.scope ?? "EXCHANGE_GLOBAL",
    bucketHash: overrides.bucketHash ?? hash(),
    windowStart: overrides.windowStart ?? windowStart,
    observedAt: overrides.observedAt ?? new Date("2026-09-13T16:00:05.000Z"),
    limit: overrides.limit ?? 10,
  } as const;
}

afterEach(async () => {
  const hashes = [...createdHashes];
...
describe("P11 Buyer Access rate-limit repository on isolated MySQL", () => {
  it("creates a new fixed-window bucket at request count 1", async () => {
    ...
    expect(await repository.consume(value)).toBe(1);

    const row = await database.buyerAccessRateLimitBucket.findFirst({
      where: {
        scope: value.scope,
  ...
  it("increments the same scope, hash, and window", async () => {
    ...
    expect(await repository.consume(value)).toBe(2);

    expect(await repository.consume(value)).toBe(3);
  });
  ...
  it("saturates the persisted counter at limit plus one", async () => {
    ...
    const row = await database.buyerAccessRateLimitBucket.findFirstOrThrow({
      where: {
        scope: value.scope,
        bucketHash: value.bucketHash,
        windowStart: value.windowStart,
  ...
  it("isolates identical hashes across scopes", async () => {
    ...
    expect(await repository.consume(first)).toBe(1);

    expect(await repository.consume(first)).toBe(2);

    expect(await repository.consume(second)).toBe(1);
  });
  ...
  it("isolates the same scope and hash across fixed windows", async () => {
    ...
    const second = input({
      ...
      observedAt: new Date("2026-09-13T16:01:05.000Z"),
    });
