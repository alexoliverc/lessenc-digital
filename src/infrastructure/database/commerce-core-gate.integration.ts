...
const SESSION_SECRET =
  "gate-b-buyer-session-secret-at-least-32-bytes-long";

const PRICE_MINOR = 2990;

const EMAIL =
  `gate-b-${randomUUID()}@example.invalid`;
...
const STORAGE_KEY =
  `gate-b/${RESOURCE_ID}.pdf`;

const RESOURCE_CONTENT =
  "L'Essenc Gate B protected commerce-core fixture.";
...
function guardedTestUrl(): string {
  ...
  if (
    ...
    throw new Error(
      "Gate B requires APP_ENV=test and TEST_DATABASE_URL",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Gate B refused a non-isolated P06 test database",
    );
  }
...
function approvedSnapshot(): ProviderSnapshot {
  return Object.freeze({
    providerOrderId:
      `ORD${randomUUID().replaceAll("-", "")}`,
    providerPaymentId:
      `PAY${randomUUID().replaceAll("-", "")}`,
...
describe(
  "Gate B commerce core on isolated MySQL",
  () => {
    beforeAll(async () => {
      execFileSync(
        process.execPath,
        [
          "scripts/p06-db-guard.mjs",
          "test",
        ],
        {
          stdio: "pipe",
      ...
      await db.product.create({
        data: {
          id: PRODUCT_ID,
          name:
            "Gate B Commerce Core",
          description:
            "Canonical Gate B integration fixture",
          status: "ACTIVE",
          offers: {
            create: {
              id: OFFER_ID,
    ...
    it(
      "completes Order -> Payment -> Entitlement -> protected Delivery safely, reliably and auditably",
      async () => {
        /*
         * ----------------------------------------------------
        ...
        expect(
          retryCheckoutInput.orderId,
        ).toBe(
          checkoutInput.orderId,
        );

        expect(
          retryCheckoutInput.customerId,
        ).not.toBe(
          checkoutInput.customerId,
        );

        expect(
          retryCheckoutInput.itemId,
        ).not.toBe(
          checkoutInput.itemId,
        );
        ...
        const reserved =
          await paymentRepository.reserve(
            ORDER_ID,
            "PIX",
          );
        ...
        const approvedPayment =
          await db.payment.findUniqueOrThrow({
            where: {
              id:
                reserved.attempt.paymentId,
            },
        ...
        if (!approvalEvent) {
          throw new Error(
            "GATE_B_PAYMENT_APPROVED_EVENT_MISSING",
          );
        }
        ...
        expect(
          approvalEvent.deduplicationKey,
        ).toBe(
          `payment-approved:${reserved.attempt.paymentId}`,
        );
        ...
        const entitlementGrant =
          new ProcessEntitlementGrant(
            new PrismaEntitlementGrantRepository(
              db,
            ),
          );

        expect(
          await entitlementGrant.execute(
            approvalEvent.id,
          ),
        ).toBe(
          "PROCESSED",
        ...
        expect(
          entitlement.sourceOutboxEventId,
        ).toBe(
          approvalEvent.id,
        );
        ...
        const issueCredential =
          new IssueBuyerAccessCredential(
            credentialRepository,
            credentialService,
          );

        const issued =
          await issueCredential.execute(
            ORDER_ID,
          );
        ...
        expect(
          persistedCredential.status,
        ).toBe(
          "ACTIVE",
        );

        expect(
          persistedCredential.activeOrderKey,
        ).toBe(
          ORDER_ID,
        );

        expect(
          persistedCredential.secretHash,
        ).not.toBe(
          issued.rawCredential,
        );
        ...
        const exchanged =
          await exchangeCredential.execute(
            issued.rawCredential,
          );

        expect(
          exchanged.sessionToken.startsWith(
            "v1.",
          ),
        ).toBe(true);

        const cryptographicSession =
          sessionService.verify(
            exchanged.sessionToken,
          );

        expect(
          cryptographicSession,
        ).toMatchObject({
          customerId:
            CUSTOMER_ID,
          orderId:
        ...
        requestHeaders.set(
          "cookie",
          `${cookieName}=${exchanged.sessionToken}`,
        );

        const request =
          new NextRequest(
            `http://localhost/api/buyer-access/resources/${RESOURCE_ID}`,
            {
              method: "GET",
              headers:
                requestHeaders,
        ...
        const deliveryEvents =
          await db.digitalDeliveryEvent.findMany({
            where: {
              entitlementId:
                entitlement.id,
              resourceId:
