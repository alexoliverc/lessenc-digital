...
const AUTHORIZED = Object.freeze({
  ...
  storageKey: "resources/recovery/v1.pdf",
...
async function temporaryRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "lessenc-p11-c64-recovery-"));
...
async function writeResource(root: string, content = "recovered-pdf"): Promise<void> {
  const path = resolve(root, AUTHORIZED.storageKey);
...
function auditFixture() {
  const records: DigitalDeliveryAuditRecord[] = [];

  return {
    records,
    repository: {
      record: vi.fn(async (record: DigitalDeliveryAuditRecord) => {
        records.push(
          Object.freeze({
            ...record,
          }),
...
async function consumeDeliveryBody(body: AsyncIterable<Uint8Array>): Promise<number> {
  let bytes = 0;
...
describe("P11 C6.4 storage failure recovery", () => {
  it("recovers on a later request after a missing authorized object is restored", async () => {
    ...
    const service = new PrepareProtectedDelivery(authorizer(), storage, audit.repository);
    ...
    const delivery = await service.execute(SUBJECT, AUTHORIZED.resourceId);

    const outcome = new RecordProtectedDeliveryOutcome(audit.repository);

    const deliveredBytes = await consumeDeliveryBody(delivery.body);

    expect(deliveredBytes).toBeGreaterThan(0);

    await outcome.succeeded(delivery);
  ...
  it("recovers with the same storage adapter after a previously unavailable root is restored", async () => {
    ...
    await new RecordProtectedDeliveryOutcome(audit.repository).succeeded(delivery);

    expect(audit.records.map((record) => record.outcome)).toEqual(["FAILED", "SUCCEEDED"]);
  });
  ...
  it("normalizes an unexpected storage outage and permits a later clean request", async () => {
    ...
    expect(delivery.resourceId).toBe(AUTHORIZED.resourceId);
  ...
  it("does not rewrite a failed delivery event when a later attempt succeeds", async () => {
    ...
    const firstRecord = audit.records[0];

    await writeResource(root, "second-attempt");
    ...
    expect(firstRecord).toEqual(audit.records[0]);

    expect(audit.records).toHaveLength(2);

    expect(audit.records[0]?.outcome).toBe("FAILED");

    expect(audit.records[1]?.outcome).toBe("SUCCEEDED");
  });
});
