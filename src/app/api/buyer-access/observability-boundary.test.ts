...
const RAW_CREDENTIAL = `lba_${"x".repeat(43)}`;

const SESSION_TOKEN = "v1.private-session.signature";
...
function exchangeRequest(): Request {
  return new Request(`${APP_URL}/api/buyer-access/exchange`, {
    method: "POST",
    headers: {
      origin: APP_URL,
...
function libraryRequest(): NextRequest {
  return new NextRequest(`${APP_URL}/api/buyer-access/library`, {
    headers: {
      cookie: `__Host-lessenc_buyer=${SESSION_TOKEN}`,
    },
  });
}

function downloadRequest(): NextRequest {
  return new NextRequest(`${APP_URL}/api/buyer-access/resources/${RESOURCE_ID}`, {
    headers: {
      cookie: `__Host-lessenc_buyer=${SESSION_TOKEN}`,
    },
  });
...
function delivery(body: AsyncIterable<Uint8Array> = successfulBody()): PreparedProtectedDelivery {
...
describe("P11 Buyer Access observability boundary", () => {
  ...
  it("logs invalid Buyer Access without credential or token disclosure", async () => {
    ...
    const response = await handler(exchangeRequest());
    ...
    expect(record.correlationId).toMatch(/^[0-9a-f-]{36}$/iu);
  });
  ...
  it("logs HTTP rate limiting without exposing raw credential material", async () => {
    ...
    const handler = createBuyerAccessExchangeHandler({
      exchange: {
        execute: vi.fn().mockRejectedValue(new BuyerAccessRateLimitExceeded(17)),
      },
      appUrl: APP_URL,
      appEnv: "production",
    });
  ...
  it("logs protected resource denial without high-cardinality identifiers", async () => {
    ...
    const response = await handler(downloadRequest(), RESOURCE_ID);
    ...
    expect(serialized).not.toContain(SUBJECT.orderId);
    expect(serialized).not.toContain(SUBJECT.credentialId);
    ...
    const handler = createProtectedDownloadHandler({
      ...
      prepareDelivery: {
        execute: vi.fn().mockRejectedValue(
          new Error("DELIVERY_UNAVAILABLE", {
            cause: new PrivateResourceStorageError("STORAGE_ROOT_UNAVAILABLE"),
          }),
        ),
      },
      recordOutcome: {
  ...
  it("logs delivery audit unavailability before releasing response bytes", async () => {
    ...
    const handler = createProtectedDownloadHandler({
      ...
      prepareDelivery: {
        execute: vi.fn().mockResolvedValue(delivery(guardedBody())),
      },
      recordOutcome: {
    ...
    expect(parseRecord(diagnostic.mock.calls[0]?.[0])).toMatchObject({
  ...
  it("logs stream failure after delivery start without logging internal exception text", async () => {
    ...
    async function* failingBody() {
      ...
      throw new Error("C:\\private\\secret-resource.pdf");
    }
    ...
    const handler = createProtectedDownloadHandler({
      ...
      prepareDelivery: {
        execute: vi.fn().mockResolvedValue(delivery(failingBody())),
      },
      recordOutcome: {
        succeeded: vi.fn().mockResolvedValue(undefined),
        streamFailed: vi.fn().mockResolvedValue(undefined),
      },
      appEnv: "production",
    });
    ...
    expect(serialized).not.toContain("secret-resource.pdf");
    expect(serialized).not.toContain(RESOURCE_ID);
