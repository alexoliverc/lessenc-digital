...
describe("P11 internal failure observability", () => {
  ...
  it("normalizes a limiter backend failure without invoking the protected operation", async () => {
    ...
    const service = new RateLimitedBuyerAccessExchangeExecutor(inner, limiter, hasher);

    const failure = await service.execute("lba_test").catch((error: unknown) => error);

    expect(failure).toBeInstanceOf(BuyerAccessRateLimitUnavailable);
    ...
    const handler = createBuyerAccessExchangeHandler({
      exchange: {
        execute: vi.fn().mockRejectedValue(
          new BuyerAccessRateLimitUnavailable("EXCHANGE_GLOBAL", {
            cause: new Error("mysql://private-user:private-password@database"),
          }),
        ),
      },
      appUrl: APP_URL,
  ...
  it("logs library limiter unavailability with the correct scope", async () => {
    ...
    const handler = createBuyerAccessLibraryHandler({
      ...
      listResources: {
        execute: vi.fn().mockRejectedValue(
          new BuyerAccessRateLimitUnavailable("LIBRARY_CREDENTIAL", {
    ...
    const response = await handler(
      new NextRequest(`${APP_URL}/api/buyer-access/library`, {
        headers: {
    ...
    expect(record(diagnostic.mock.calls[0]?.[0])).toMatchObject({
      event: "buyer_access_limiter_unavailable",
      surface: "RATE_LIMIT",
  ...
  it("logs download limiter unavailability with the correct scope", async () => {
    ...
    const handler = createProtectedDownloadHandler({
      ...
      prepareDelivery: {
        execute: vi.fn().mockRejectedValue(
          new BuyerAccessRateLimitUnavailable("DOWNLOAD_CREDENTIAL", {
            cause: new Error("database unavailable"),
          }),
        ),
      },
    ...
    const response = await handler(
      new NextRequest(`${APP_URL}/api/buyer-access/resources/${RESOURCE_ID}`, {
        headers: {
          cookie: "__Host-lessenc_buyer=session-token",
        },
      }),
  ...
  it("logs credential revocation failure without order or credential identifiers", async () => {
    ...
    const repository = {
      revoke: vi.fn().mockRejectedValue(new Error(`private failure ${ORDER_ID} ${CREDENTIAL_ID}`)),
      reissue: vi.fn(),
    };

    const service = new RevokeBuyerAccessCredential(repository);

    await expect(service.execute(ORDER_ID, CREDENTIAL_ID)).rejects.toThrow();
    ...
    expect(record(serialized)).toMatchObject({
      event: "credential_recovery_failed",
  ...
  it("logs credential reissue failure without exposing generated credential material", async () => {
    ...
    const rawCredential = `lba_${"z".repeat(43)}`;

    const repository = {
      revoke: vi.fn(),
      reissue: vi.fn().mockRejectedValue(new Error(`private failure ${rawCredential}`)),
    };
    ...
    const secretService = {
      ...
      hash: vi.fn().mockReturnValue({
        ok: false as const,
        reason: "MALFORMED_CREDENTIAL" as const,
      }),
    };

    const service = new ReissueBuyerAccessCredential(repository, secretService);
  ...
  it("logs refund entitlement revocation failure without exposing the outbox event id", async () => {
    ...
    const service = new ProcessEntitlementRevocation({
      processRefundCompleted: vi
        .fn()
        .mockRejectedValue(new Error(`persistence failure ${OUTBOX_EVENT_ID}`)),
    });

    await expect(service.execute(OUTBOX_EVENT_ID)).rejects.toThrow();
