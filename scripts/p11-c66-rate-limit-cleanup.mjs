...
import { logger } from "../src/lib/observability/logger.ts";
...
function requireBatchSize() {
  const raw = argumentValue("batch-size");
...
function requireDatabaseConfiguration() {
  ...
  if (!rawUrl) {
    throw new Error("DB_RUNTIME_URL_REQUIRED");
  }

  if (!caFile) {
    throw new Error("DB_TLS_CA_FILE_REQUIRED");
  }

  if (!["local", "test", "production"].includes(appEnv ?? "")) {
  ...
  if (!expectedDatabase || expectedDatabase !== database) {
    throw new Error("RATE_LIMIT_CLEANUP_TARGET_MISMATCH");
  }

  if (appEnv === "production" && !hasFlag("allow-production")) {
    throw new Error("PRODUCTION_CLEANUP_REQUIRES_EXPLICIT_AUTHORIZATION");
  }
...
async function countStale(pool, cutoff) {
  ...
  return safeCount(rows[0]?.count, "INVALID_RATE_LIMIT_STALE_COUNT");
}

async function deleteBatch(pool, cutoff, limit) {
  ...
  return safeCount(result.affectedRows, "INVALID_RATE_LIMIT_CLEANUP_COUNT");
}
...
try {
  ...
  const cutoff = new Date(observedAt.getTime() - RETENTION_SECONDS * 1000);
   ...
   */
  const cutoffSql = mysqlUtcDateTime(cutoff);

  pool = mariadb.createPool(configuration.connection);

  const staleBefore = await countStale(pool, cutoffSql);
  ...
  if (!execute) {
    ...
    while (remaining > 0) {
      ...
      const deleted = await deleteBatch(pool, cutoffSql, limit);
    ...
    const staleAfter = await countStale(pool, cutoffSql);

    if (staleAfter > 0) {
      logger.warn("rate_limit_stale_buckets_detected", {
        correlationId,
        surface: "RATE_LIMIT",
        outcome: "DEGRADED",
        failureCode: "RATE_LIMIT_STALE_BUCKETS_DETECTED",
        windowSeconds: RETENTION_SECONDS,
        limit: batchSize,
      });
    }
