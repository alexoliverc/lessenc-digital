...
**Estado atual:** P14-01–P14-06 possuem `IMPLEMENTATION CANDIDATE — AWAITING INDEPENDENT AUDIT` não commitado em `phase/p14-security-hardening`. A correção R1 removeu a regressão de restringir 3DS a hosts Mercado Pago: PIX mantém allowlist estrita, enquanto challenge 3DS exige proveniência do provider server-side fixo, HTTPS seguro, hostname não local/literal, estado canônico e associação financeira local antes da exposição. A CSP global continua restrita e somente `/checkout/payment` admite `frame-src https:` para ACS dinâmico; `script-src-attr 'none'` foi adicionado. O workflow passou a auditar o grafo completo, mas Actions/CodeQL/Dependabot permanecem `IMPLEMENTED AS CODE / GITHUB EXECUTION VALIDATION PENDING`. Regressão local R1: 689/689 unit/application, 190/190 MySQL, lint, typecheck, build, audit completo e smoke CSP público/checkout PASS. Schema/migrations/dependências/lockfile inalterados. P14 não está COMPLETE, MERGED ou CHECKPOINTED; deploy permanece NOT PERFORMED.
